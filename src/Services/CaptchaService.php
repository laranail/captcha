...
final class CaptchaService
{
    ...
    {
        $this->adapter = new CaptchaFake($verifies);

        return $this;
    }

    /** Verify successfully, but with a score — for exercising the review band. */
    public function fakeScore(float $score): self
    {
        $this->adapter = new CaptchaFake(score: $score);

        return $this;
    }

    /**
     * Answer a scripted sequence, one result per call.
     *
    ...
    {
        $this->adapter = (new CaptchaFake)->queue($results);

        return $this;
    }

    /**
     * Assertions delegate to the installed fake.
     *
     * They live here so `fake()` can keep returning the service — its signature is `self` and the
     * facade advertises that, so handing back the double instead would be a silent breaking change
     * for anyone chaining off it. The assertions themselves are implemented in `src/Testing`,
    ...
    {
        $this->installedFake()->assertVerified($matching);
    ...
    {
        $this->installedFake()->assertFailed($matching);
    ...
    {
        $this->installedFake()->assertNothingVerified();

        return $this;
    }

    public function assertVerifiedCount(int $expected): self
    {
        $this->installedFake()->assertVerifiedCount($expected);
    ...
    private function installedFake(): CaptchaFake
    {
        ...
        if (! $adapter instanceof CaptchaFake) {
            throw new RuntimeException(
                'Call Captcha::fake() before asserting against captcha verifications.',
            );
        }
